...
public class BatchConfig {
  ...
  // Input part
  // for reading : MatchInput Instance; for writing: Match instance
  @Bean
  public FlatFileItemReader<MatchInput> reader() {
    return new FlatFileItemReaderBuilder<MatchInput>().name("MatchItemReader")
        ...
        .fieldSetMapper(new BeanWrapperFieldSetMapper<MatchInput>() {
          {
            setTargetType(MatchInput.class); // create class instance from each field_names values
          }
        }).build();
  }

  // process part to call the processor to convert the input in desire type.
  @Bean
  ...
  // 10 items per chunks reading/processing/writing
  @Bean
